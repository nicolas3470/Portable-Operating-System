/*
 * network.c:
 *	This module provides a simplified, interrupt-driven interface to the network.
 */
...
send_pkt(network_address_t dest_address, 
   ...
   */
  bufp = (char *) malloc(pktlen);
  memcpy(bufp, hdr, hdr_len);
  memcpy(bufp + hdr_len, data, data_len);
