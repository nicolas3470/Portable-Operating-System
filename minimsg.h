...
#define MAX_UNRELIABLE_DATA_SIZE (MAX_NETWORK_PKT_SIZE - sizeof(struct mini_header) - 181)

typedef char* minimsg_t;

typedef struct miniport* miniport_t;

extern queue_t minithreads_waiting_to_receive;
extern miniport_t miniport_array[65536];
extern network_address_t my_address;
...
typedef struct received_msg{
	network_address_t foreign_address;
	unsigned short foreign_listening_port;
	char data[MAX_UNRELIABLE_DATA_SIZE];
}* received_msg_t;
